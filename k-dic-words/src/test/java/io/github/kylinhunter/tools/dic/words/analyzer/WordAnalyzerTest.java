...
class WordAnalyzerTest {
    ...
    void test() {
        ...
        Words words = CF.get(IKWordAnalyzer.class).analyze(text);
        words.forEach(System.out::println);
        Words words2 = CF.get(IKWordAnalyzer.class).analyze(text);
        words2.forEach(System.out::println);
        Assertions.assertEquals(words2.size(), words.size());
    }
}